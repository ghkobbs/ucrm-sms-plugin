...
use SmsNotifier\Facade\MBNotifierFacade;
...
class Plugin
{
    ...
    public function __construct(
        ...
        MBNotifierFacade $notifierFacade,
    ...
    public function run(): void
    {
        if (PHP_SAPI === 'fpm-fcgi') {
            $this->logger->info('Message Bird SMS over HTTP started');
            $this->processHttpRequest();
        } elseif (PHP_SAPI === 'cli') {
            $this->logger->info('Message Bird SMS over CLI started');
            $this->processCli();
        } else {
