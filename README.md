# Ucrm SMS notifier - Message Bird
...
* [Message Bird](https://www.messagebird.com/) account is required to access its API.

## Configuration

* Install the plugin into UCRM and enable it. I.e. download the plugin [zip file](https://github.com/ghkobbs/ucrm-sms-plugin/raw/master/sms-message-bird.zip) and upload it to UCRM in System > Plugins.
* Keep execution period at "don't execute automatically" - the plugin will react to webhook events.
* Set up with access key which you obtain from [Message Bird](https://dashboard.messagebird.com/en-au/developers/access)

    
Note: there are two sets of credentials available, the default ("LIVE credentials") for actual use and test credentials for development.
...
* Enable the plugin
* Click Add webhook (button next to Public URL)
...
* Click Test Endpoint
* Go to System / Plugins / SMS notifications via Message Bird
